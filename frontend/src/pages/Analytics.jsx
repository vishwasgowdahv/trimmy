...
const Analytics = () => {
  ...
  const deviceStats = [
    {
      name: "Desktop",
      value: stats?.desktop_clicks || 0,
      icon: Monitor,
      color: "bg-blue-500",
    },
    {
      name: "Mobile",
      value: stats?.mobile_clicks || 0,
      icon: Smartphone,
      color: "bg-purple-500",
    },
    {
      name: "Tablet",
      value: stats?.tablet_clicks || 0,
      icon: Tablet,
      color: "bg-indigo-500",
    },
  ];
  ...
  const timeline = Array.from({ length: 7 })
    .map((_, i) => {
      const date = subDays(new Date(), i);
      const count =
        clicks?.filter((c) => isSameDay(new Date(c.clicked_at), date)).length ||
        0;
      return { date, count };
    })
    .reverse();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="space-y-2">
            <Link
              to="/dashboard"
              className="inline-flex items-center gap-2 text-gray-500 hover:text-blue-600 transition-colors font-bold text-sm mb-2"
            >
              <ArrowLeft size={16} />
              Back to Dashboard
            </Link>
            ...
            <div className="px-4 py-2 bg-gray-50 rounded-xl">
              ...
              <p className="text-sm font-bold text-gray-700">
                {stats?.last_clicked_at
                  ? format(new Date(stats.last_clicked_at), "MMM dd")
                  : "Never"}
              </p>
            </div>
          </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
              ...
              <div className="flex items-end justify-between gap-2 h-48">
                {timeline.map((day, idx) => (
                  <div
                    key={idx}
                    className="grow flex flex-col items-center gap-3 group"
                  >
                    <div className="w-full relative h-full flex items-end justify-center">
                      ...
                      <div
                        ...
                        style={{
                          height: `${(day.count / maxTimelineCount) * 100}%`,
                  ...
                  <div key={idx} className="space-y-2">
                    ...
                    <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${stats?.total_clicks > 0 ? (device.value / stats.total_clicks) * 100 : 0}%`,
                        }}
                      />
                    </div>
                  </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
              ...
              <div className="space-y-3">
                {topReferrers.map(([ref, count], idx) => (
                  ...
                  >
                    <span className="text-sm font-bold text-gray-500 truncate max-w-[150px]">
                      {ref}
                    </span>
                    <div className="flex items-center gap-2">
                      <div className="h-1 w-20 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(count / (stats?.total_clicks || 1)) * 100}%`,
                          }}
                        />
                      </div>
